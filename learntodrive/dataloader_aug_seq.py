...
def augment_image(img, mask, translation_x, translation_y, p_flip, p_aug1, p_aug2, p_aug3, s, p=0.33):
    """
    ...
    if p_aug1 <= 0.1:
        aug_img = change_image_brightness_rgb(aug_img, s)
...
class Drive360(object):
    ...
    def __getitem__(self, index):
        ...
        p_aug1 = random.random()
        p_aug2 = random.random()
        p_aug3 = random.random()
        s = np.random.uniform(0.2, 0.75)
        ...
        if self.front:
            ...
            for row_idx, (_, row) in enumerate(rows.iterrows()):
                ...
                img_front_mask = Image.open('../semantic-segmentation/sample3_mask_1_real/' + row['cameraFront'])
                img_front_mask_tmp = np.asarray(img_front_mask)
                
                if aug <= self.p_aug:
                    img_front_np, img_front_mask_tmp, angle_sign, angle_shift = augment_image(img_front_np, 
                                                                                              img_front_mask_tmp, 
                                                                                              translation_x, 
                                                                                              translation_y, 
                                                                                              p_flip, 
                ...
                #tmp_front = Image.open(self.data_dir + row['cameraFront'])
                #tmp_img = Image.open('../semantic-segmentation/sample3_mask_1_real/' + row['cameraFront'])
                #tmp_front_aug, mask_aug, angle_sign = augment_image(np.asarray(tmp_front), tmp_img_2)
                #inputs['cameraFront'][row_idx] = (self.imageFront_transform(tmp_img))
                #tmp_img_2 = torch.tensor(mask_aug)
        if self.right_left:
        ...
        #tmp_steering_denormalized = (tmp_steering*target_std['canSteering'])+target_mean['canSteering']
        #if tmp_steering_denormalized >= higher:
        #    steering_class = 1
        #else:
        #    if tmp_steering_denormalized <= lower:
        #        steering_class = 2
        #    else:
        #        steering_class = 0
        #labels['canSpeed'] = self.dataframe['canSpeed'].iloc[index]
        #labels['canSteering'] = steering_class
