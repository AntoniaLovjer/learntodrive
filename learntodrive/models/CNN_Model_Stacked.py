...
class DenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
        ...
        self.features = nn.Sequential(OrderedDict([
            ('conv0', nn.Conv2d(23*10, num_init_features, kernel_size=7, stride=2,
    ...
    def forward(self, x, hidden_features=None):
        ff = []
        for i in range(10):
            ff.append(x['cameraFront'][i])
            ff.append(x['cameraFront_mask'][i])
        x = torch.cat(ff, 1)
        features = self.features(x)
        out = F.relu(features, inplace=True)
        out = F.adaptive_avg_pool2d(out, (1, 1))
        out = torch.flatten(out, 1)
        prediction = {'canSteering': torch.squeeze(self.control_angle(out)),
                      'canSpeed': torch.squeeze(self.control_speed(out))}
        return prediction
